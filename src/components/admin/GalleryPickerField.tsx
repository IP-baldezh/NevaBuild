...
import { ImageIcon, Upload, X, Check, Loader2, Images, Plus } from "lucide-react";
...
export function GalleryPickerField({ name, defaultValue = [] }: Props) {
  ...
  return (
    <div className="space-y-2">
          ...
          {urls.map((url, i) => (
            <div key={i} className="relative group">
              ...
              <img
                src={url}
                alt=""
                className="size-20 rounded-lg border border-gray-200 bg-gray-50 object-contain"
              />
              <button
                type="button"
      ...
      <button
        type="button"
        onClick={openDialog}
        className="flex items-center gap-2 h-9 rounded-lg border-2 border-dashed border-gray-200 px-4 text-sm text-gray-400 transition-colors hover:border-gray-400 hover:text-gray-600"
      >
        <Plus className="size-4" />
        {urls.length > 0 ? "Добавить ещё" : "Добавить изображение"}
      </button>
